...
{
    // Compare two Stores POINTED TO by lhs and rhs
...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    vector<Store*> ptrs;
        ...
        // Create a vector of Store pointers
        for (auto& store : stores)
            ptrs.push_back(&store);

        // Sort the vector of pointers
        sort(ptrs.begin(), ptrs.end(), comp);

        // Rearrange the Stores according to the sorted pointers
        auxStores.reserve(stores.size());
        for (auto* ptr : ptrs)
            auxStores.push_back(*ptr);

        // Replace the stores with the sorted Stores
        stores = std::move(auxStores);
}
