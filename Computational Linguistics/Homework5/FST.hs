...
-- simply return the input as the only option
return :: a -> [a]
return = \a -> [a]
...
-- a variant of relation composition that compresses the final results
(>>) :: (Eq c, Semiring w) => (a -> [b]) -> (b -> [(c,w)]) -> (a -> [(c,w)])
f >> g = \a -> compress [c | b <- f a, c <- g b]

walk' :: (Eq c, Semiring w) => FST c w -> [c] -> (State, w) -> [(State, w)]
walk' fst u = go return u
  where go steps []     = steps
        go steps (c:cs) = go (steps >> step fst c) cs
